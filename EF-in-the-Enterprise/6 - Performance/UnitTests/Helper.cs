...
namespace ContosoUniversity.UnitTests
{
    ...
    {
        public static void DataInitialize(string userLastName = "")
        {
            // Disable data migration.
            Database.SetInitializer<SchoolContext>(null);

            // Read list of scripts from Post Deployment script.
            ...
            using (var cn = new SqlConnection(ConfigurationManager.ConnectionStrings["SchoolContext"].ConnectionString))
            {
                ...
                {
                    var updatePerson = new SqlCommand("UPDATE Person SET UserName = @UserName WHERE LastName = @LastName", cn);
                    updatePerson.Parameters.AddWithValue("@UserName", Thread.CurrentPrincipal.Identity.Name);
                    updatePerson.Parameters.AddWithValue("@LastName", userLastName);
        ...
        public const long TimeStandard = 1000;
